...
async function main() {
    ...
    // Block epoch
    const epochLengthInBlocks = '16200'; //'9600'; // 8+1 hr in BSC
    const firstEpochBlock = ((blockNumber * 1) + (30 * 60 / 2)) + ''; // next 30 min 12835700
    ...
    const dai = await ERC20Token.deploy("DAI Token", "DAI", 18, "10000000000000000000000000"); // mint 10m BUSD
    ...
    const treasury = await Treasury.deploy(templar.address, dai.address, 0); // _blocksNeededForQueue = 0
    ...
    const busdBond = await DAIBond.deploy(templar.address, dai.address, treasury.address, DAO.address, zeroAddress);
    ...
    await dai.approve(treasury.address, maxUint256);
    await treasury.deposit(depositBUSD, dai.address, reservedBUSD); // mint 500 TEM
    ...
    console.log( "DAIBond: " + busdBond.address );
}
